...
describe("Test redis-trie-vi", function() {
  it('.add("user1")', function(done) {
    rtrie
      .add(user1.username, user1, user1._id, dateFromObjectId(user1._id))
      .then(function() {
        done();
      })
      .catch(done);
  ...
  it('.add("user2")', function(done) {
    rtrie
      .add(user2.username, user2, user2._id, dateFromObjectId(user2._id))
      .then(function() {
        done();
      })
      .catch(done);
